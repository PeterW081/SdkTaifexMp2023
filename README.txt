《專案介紹》
　　本案建置基於《臺灣期貨交易所 TCP/IP TMP 電文規範》的「第 2.15.0 版」，用以簡化期貨交易系統的開發過程。目前因為缺乏時間和缺乏測試環境而中斷，希望以後能有機會完善。上述《電文規範》後續稱為 TCP 報文協定，存有副本於專案檔案中的 <RESOURCE> 目錄。

　　該個專案目前主要內容，乃是基於上述 TCP 報文協定提供 API。在保證程式代碼的易維護性下，儘量提升程式代碼的運行效能，用以適應高頻交易之類的交易策略。程式代碼特點簡述如下：
　　物件導向設計，對於上述 TCP 報文協定中的報文，分別包裝了 Raw Object 和 Reference Objecet。提升程式碼可讀性以及便利 Code Review 時候檢查代碼。其中類似 L41 的不定長度的 TCP 報文，花了一番功夫進行設計。
