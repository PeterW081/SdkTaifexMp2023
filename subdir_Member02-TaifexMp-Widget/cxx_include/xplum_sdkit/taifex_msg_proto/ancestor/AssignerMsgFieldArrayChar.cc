...
class xplum_sdkit::taifex_msg_proto::ancestor::AssignerMsgFieldArrayChar : protected _nshdr::AssignerArrayChar<VALUE_ARRAY_SIZE>
{
  ...
  public:
    ...
    static auto Assign(Type (&lvariable)[M_SIZE], std::span<Type, RVALUE_ARRAY_SIZE> rvalue) -> std::size_t;
    template <std::size_t RVALUE_ARRAY_SIZE>
    static auto Assign(Type (&lvariable)[M_SIZE], const std::array<Type, RVALUE_ARRAY_SIZE> &rvalue) -> std::size_t;
