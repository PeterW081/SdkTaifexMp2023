...
//
auto nscxx_root::CounterOrderSeqSimple::storage_load0(std::nullptr_t melon, ...) -> bool {
...
auto nscxx_root::CounterOrderSeqSimple::storage_save0(std::nullptr_t melon, ...) const -> bool {
  std::unique_ptr<type::uint32> *order_no;
  std::map<type::uint32, type::uint32> *map_K_order_no_V_order_id;
...
auto nscxx_root::CounterOrderSeqSimple::storage_load(std::unique_ptr<type::uint32> *order_no, std::map<type::uint32, type::uint32> *map_K_order_no_V_order_id) -> bool {
...
auto nscxx_root::CounterOrderSeqSimple::storage_save(std::unique_ptr<type::uint32> *order_no, std::map<type::uint32, type::uint32> *map_K_order_no_V_order_id) const -> bool {
  if (order_no == nullptr || map_K_order_no_V_order_id == nullptr) {
    assert(!(order_no == nullptr));
    assert(!(map_K_order_no_V_order_id == nullptr));
    return false;
...
//
auto ConvertorOrderNo::FX_CONVERT_ORDER_NO_TO_VALUE(std::string_view txt_value) -> std::optional<std::uint32_t> { return To_Value(txt_value); }
auto ConvertorOrderNo::FX_CONVERT_ORDER_NO_TO_TEXT_STRING(std::uint32_t int_value) -> std::string { return To_String(int_value); }
