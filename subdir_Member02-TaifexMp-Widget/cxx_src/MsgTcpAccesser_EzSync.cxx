...
// using xplum_sdkit::taifex_msg_proto::network::MsgTcpAccesser_EzSync;
//
nscxx_root::MsgTcpAccesser_EzSync::MsgTcpAccesser_EzSync(std::string_view server_host, std::string_view server_port) : ext::boost_asio0::EzTcpAccesserWay01SyncSimple(server_host, server_port) {
  dynamic_cast<EzTcpAccesserWay01SyncSimple *>(this)->DEBUG_cfg.is_log_data_hex_content = false;
}
auto nscxx_root::MsgTcpAccesser_EzSync::write_msg(std::span<std::byte> buffer) -> xplum_model::taifex_msg_proto::type::TypeMsgLength {
  ...
  this->write(buffer.subspan(0,                                                                 //
                             (                                                                  //
                                 mt_sizeof_msg_read_buffer::SIZE_MIN_OF_READ_MSG +              //
                                 ez_view::FX_GET_MSG_LENGTH_IN_ORIGIN_MSG_1CONST(buffer.data()) //
                                 )                                                              //
                             ));
...
auto nscxx_root::MsgTcpAccesser_EzSync::reade_msg(std::span<std::byte> buffer) -> xplum_model::taifex_msg_proto::type::TypeMsgLength {
  type::TypeMsgLength msg_length;

  assert(buffer.size() >= mt_sizeof_msg_read_buffer::SIZE_MIN_OF_READ_MSG);
  this->reade(buffer.subspan(0, mt_sizeof_msg_read_buffer::SIZE_MIN_OF_READ_MSG));
  ...
  this->reade(buffer.subspan(mt_sizeof_msg_read_buffer::SIZE_MIN_OF_READ_MSG, msg_length));
