...
template <EnumFuncXXX FUNC_XXX>
static inline auto BOOST_ASIO_ASYNC_BOTH_FUNC_XXX(std::optional<std::chrono::seconds> &, io_context &, tcp::socket &, std::span<std::byte> &) -> std::size_t;
template <typename T>
static inline auto BOOST_ASIO_ASYNC_RUN_ONE_WITH_TIMEOUT(std::optional<std::chrono::seconds> &, io_context &, std::optional<T> &, std::function<void()>) -> void;
}
