...
// ******************************** ******************************** ******************************** ******************************** //
// AssignerArray
template <typename T01, std::size_t V02> auto xplum::ancestor::AssignerArray<T01, V02>::Fill(Type (&lvariable)[M_SIZE], Type filler) -> std::size_t {
  std::memset(lvariable, filler, M_SIZE);
  return M_SIZE;
}

template <typename T01, std::size_t V02> template <std::size_t RVALUE_ARRAY_SIZE> auto xplum::ancestor::AssignerArray<T01, V02>::Assign(Type (&lvariable)[M_SIZE], std::span<Type, RVALUE_ARRAY_SIZE> rvalue) -> std::size_t {
...
// ******************************** ******************************** ******************************** ******************************** //
template <std::size_t V01> template <std::size_t RVALUE_ARRAY_SIZE> auto xplum::ancestor::AssignerArrayChar<V01>::Assign(Type (&lvariable)[M_SIZE], std::span<Type, RVALUE_ARRAY_SIZE> rvalue) -> std::size_t { return SUPER::Assign(lvariable, rvalue); }
...
template <std::size_t V01> auto xplum::ancestor::AssignerArrayChar<V01>::Assign(Type (&lvariable)[M_SIZE], std::string_view rvalue) -> std::size_t { return SUPER::Assign(lvariable, std::span(const_cast<Type *>(rvalue.data()), rvalue.size())); }

template <std::size_t V01> auto xplum::ancestor::AssignerArrayChar<V01>::Advanced_Assign(const typename SUPER::AdvancedOption &&option, Type (&lvariable)[M_SIZE], std::string_view rvalue) -> std::size_t {
