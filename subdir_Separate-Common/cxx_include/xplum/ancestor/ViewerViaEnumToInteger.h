...
template <typename TYPENAME_ENUM, typename TYPENAME_INTEGER> void xplum::ancestor::ViewerViaEnumToInteger<TYPENAME_ENUM, TYPENAME_INTEGER>::operator_assign(EnumType var) { this->m_data = static_cast<EnumUnderlyingType>(var); }

template <typename TYPENAME_ENUM, typename TYPENAME_INTEGER> auto xplum::ancestor::ViewerViaEnumToInteger<TYPENAME_ENUM, TYPENAME_INTEGER>::integer_type_value() const -> IntegerType { return m_data; }

template <typename TYPENAME_ENUM, typename TYPENAME_INTEGER> auto xplum::ancestor::ViewerViaEnumToInteger<TYPENAME_ENUM, TYPENAME_INTEGER>::enum_value() const -> EnumType { return static_cast<EnumType>(static_cast<EnumUnderlyingType>(m_data)); }

template <typename TYPENAME_ENUM, typename TYPENAME_INTEGER> auto xplum::ancestor::ViewerViaEnumToInteger<TYPENAME_ENUM, TYPENAME_INTEGER>::enum_underlying_value() const -> EnumUnderlyingType { return m_data; }
